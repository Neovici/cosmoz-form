...
`,lr=()=>st,cr=rt(lr,{styleSheets:[ar]});customElements.define("cosmoz-spinner",cr);const R=e=>e??st;function D(e,t,i){return e?t(e):i?.(e)}const ur=({slot:e,title:t,className:i,width:s="24",height:n="24",styles:r}={})=>C`
	<svg
		slot=${R(e)}
		class=${`clear-icon ${i??""}`}
		viewBox="0 0 24 24"
...
`,dr=({slot:e,title:t,className:i,width:s="24",height:n="24",styles:r}={})=>C`
	<svg
		slot=${R(e)}
...
`,hr=({slot:e,title:t,className:i,width:s="24",height:n="24",styles:r}={})=>C`
	<svg
		slot=${R(e)}
...
`,pt=ft(class extends ht{update(){return this.state.host}}),lt=(e,...t)=>e.flatMap((i,s)=>[i,t[s]??""]).join(""),di=(...e)=>{const t=new CSSStyleSheet;return t.replaceSync(e.join("")),t},fr=lt`
	:host {
		display: flex;
...
`,pr=()=>{const e=pt(),t=E(()=>{e.dispatchEvent(new Event("close")),e.onClose?.()},[]);return k(()=>{const i=o=>{o.preventDefault(),t()},s=e.shadowRoot,n=o=>o.target.value==="cancel"&&i(o),r=o=>!o.defaultPrevented&&o.key==="Escape"&&i(o);return s.addEventListener("click",n),document.addEventListener("keydown",r,!0),()=>{s.removeEventListener("click",n),document.removeEventListener("keydown",r,!0)}},[]),{close:t}},gr=()=>{const e=pt(),{manualFocus:t}=e;Ge(()=>{!t&&!e.matches(":focus-within")&&(e.setAttribute("tabindex","-1"),e.focus(),e.removeAttribute("tabindex"))},[t])},mr=(e,t,i)=>{const s=e.width/3,n=e.height/3,r=Math.min(window.innerWidth-2*s,Math.max(-s,t)),o=Math.min(window.innerHeight-2*n,Math.max(-n,i));return{x:r,y:o}},_r=(e,t,i,s)=>n=>{if(!n.target.closest(t))return;const r=mr,o=e.getBoundingClientRect(),a=n.clientX-o.x,l=n.clientY-o.y,c=(h,f)=>{const p=h-a,m=f-l,g=r(o,p,m);Object.assign(e.style,{left:g.x+"px",top:g.y+"px",transform:"initial"})},u=h=>c(h.clientX,h.clientY),d=h=>{document.removeEventListener("mousemove",u),document.removeEventListener("mouseup",d)};document.addEventListener("mousemove",u),document.addEventListener("mouseup",d)},yr=()=>{const e=pt(),{unmovable:t}=e;k(()=>{if(t)return;const i=e.shadowRoot;if(!i)return;const s=_r(e,".title");return i.addEventListener("mousedown",s),()=>i.removeEventListener("mousedown",s)},[t])},br=()=>{pr(),yr(),gr()},vr=({title:e,content:t,styles:i,closeable:s=!1})=>{const n=pt();return C`
		<style>
			${fr}${i}
		</style>
	...
	`},hi=(e,{observedAttributes:t,styles:i,...s}={})=>rt(n=>(br(),vr({title:n.heading||n.title,content:e(n),styles:i,closeable:n.closeable})),{observedAttributes:["title","heading","manual-focus","unmovable","closeable",...t??[]],...s});customElements.define("cosmoz-dialog-loading",hi(()=>C`
			<style>
				.content {
					flex-direction: row;
					align-items: center;
